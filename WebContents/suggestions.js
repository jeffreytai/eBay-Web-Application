    ...
    if (sTextboxValue.length > 0) {
        ...
        xmlHttp.onreadystatechange = function() {
            if (xmlHttp.readyState === 4 && xmlHttp.status === 200) {
                addSuggestion(oAutoSuggestControl);
            }
        }
        xmlHttp.send(null);
    } else {
...
function addSuggestion(oAutoSuggestControl) {
    var aSuggestions = [];
    var cs = xmlHttp.responseXML.getElementsByTagName("CompleteSuggestion");
    for (var x=0; x<cs.length; x++) {
        aSuggestions.push(cs[x].childNodes[0].getAttribute("data"));
    }
